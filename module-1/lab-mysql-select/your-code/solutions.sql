...
SELECT authors.au_id AS "Authors ID", authors.au_lname AS "Last Name", authors.au_fname AS "First Name", publishers.pub_name AS "Publisher", COUNT(titles.title) AS "Title Count" 
FROM authors
INNER JOIN titleauthor ON authors.au_id = titleauthor.au_id
INNER JOIN titles ON titleauthor.title_id = titles.title_id
INNER JOIN publishers ON titles.pub_id = publishers.pub_id

GROUP BY author.au_id,authors.au_lname,authors.au_fname, publishers.pub_name;

-- Challenge nº3
-- Now the query should give as the authors with the highest number of titles.

SELECT authors.au_id AS "Authors ID", authors.au_lname AS "Last Name", authors.au_fname AS "First Name", publishers.pub_name AS "Publisher", SUM(titles.title) AS "Total"
FROM authors
INNER JOIN titleauthor ON authors.au_id = titleauthor.au_id
INNER JOIN titles ON titleauthor.title_id = titles.title_id
INNER JOIN publishers ON titles.pub_id = publishers.pub_id
